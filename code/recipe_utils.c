...
// 텍스트 파일에서 Recipe 구조체로 데이터를 읽어오는 함수
// recipe가 가리키는 Recipe 구조체에 파일에서 읽어온 값을 저장함 
// 읽어올 텍스트 파일의 형식은 예시 데이터 참고, create_recipe_file()에서 만드는 형식대로임 
void read_recipe_from_file(const char *code, RECIPE *recipe)
{
    char filepath[MAX_LENGTH]; 
    snprintf(filepath, MAX_LENGTH, "../data/archive/%s.txt", code);
    ...
    while (token != NULL)
    {
        ...
        if (open_paren != NULL && close_paren != NULL)
        {
            *open_paren = '\0';
            *close_paren = '\0';
            ingredient->name = malloc(strlen(token) + 1); // 이름 
            ...
            ingredient->nutrient = return_nutrient(trim_whitespace(token)); // 영양소 

            float quantity;
            char unit[50];
            sscanf(open_paren + 1, "%f%s", &quantity, unit);

            ingredient->quantity = malloc(sizeof(float)); // 재료의 양 
            ...
            *(ingredient->quantity) = quantity;
            ...
            ingredient->unit = malloc(strlen(unit) + 1); // 재료의 단위 (현재는 g만 사용-)
...
// recipe 객체에서 출력할 줄들을 저장하는 배열 만드는 함수, public에서 사용 
void store_lines_public(char **lines, int *line_count, RECIPE *recipe)
{
    ...
    for (int i = 0; i < recipe->ingre_arr.count; i++)
    {
        ...
        snprintf(buffer, MAX_LENGTH, "%-30s %10.2f%s", ingredient->name, *ingredient->quantity, ingredient->unit); // 정렬 후 출력 
        lines[(*line_count)++] = strdup(buffer);
    }
    ...
    lines[(*line_count)++] = strdup("");
    if (recipe->comments.count == 0) // 댓글이 없는 경우 
    {
    ...
    {
        strcat(buffer, "#"); //  태그에는 #을 붙여서 출력 
        strcat(buffer, recipe->tag.arr[i]);
        if (i < recipe->tag.count - 1)
        {
            strcat(buffer, " ");
        }
    }
...
void display_recipe_public(RECIPE *recipe)
{
    ...
    while (1)
    {
        ...
        for (int i = start_line; i < end_line; i++) // 한번에 30줄씩 출력하도록 함 
        {
            printf("%s\n", lines[i]);
        }
        ...
        else if (input == 'd' && end_line < line_count) // 다음 페이지 
        {
            page_index++;
        }
        else if (input == 'u' && page_index > 0) // 이전 페이지 
        {
            page_index--;
        }
        else if (input == 'q') // 나가기 
        ...
        else if (input == 's' && !save_pressed) // 저장 
        {
            save_pressed = 1;
...
void add_comment(char *code, const char *new_comment)
{
    ...
    FILE *file = fopen(filename, "r");
    if (file == NULL)
    { // 파일이 없을 때는 무시 -> private과 public 때문에 만듦
        return;
    }
...
// unique_code.txt에서 고유 코드를 읽고 1 증가시켜 반환하는 함수 (기존 파일의 값도 변환함)
char *get_and_increment_unique_code()
{
    ...
    while (1)
    {
        ...
        if (stage == 1)
        {
            ...
            while (1)
            {
                ...
                if (strcmp(ingredient, "finish") == 0) // 입력 끝 
                {
                    break;
                }
                else if (strcmp(ingredient, "modify") == 0) // 전 입력으로 돌아가서 수정 
                {
                ...
                *close_paren = '\0';
                ...
                if (!is_ingredient_in_csv(ingredient))
                { // 재료 이름이 리스트에 없을 경우 문제 출력 
        ...
        if (stage == 4)
        {
            ...
            while (1)
            {
                ...
                while (referenced_code != NULL)
                {
                    ...
                    add_code_to_referencing_section(code, referenced_code); // 코드를 참조할 경우 참조한 레시피에 표시
